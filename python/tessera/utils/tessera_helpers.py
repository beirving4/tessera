...
# Import tessera module (we're inside the package now, use parent)
# Use lazy import pattern to avoid circular imports
ts = None
HAS_TESSERA = False

def _get_tessera():
    """Lazy import of tessera to avoid circular imports."""
    global ts, HAS_TESSERA
    if ts is not None:
        return ts
    try:
        # Try the native module directly first
        import _tessera as _ts
        ts = _ts
        HAS_TESSERA = True
    except ImportError:
        try:
            # Fall back to trying tessera package
...
def load_snapshot(
    ...
    if snapshot_num is not None:
        snap = _ts.io.read_gadget4_snapshot(
            str(path),
...
def load_halo_catalog(catalog_path: str | Path) -> HaloCatalog:
    ...
    return _ts.io.read_gadget4_halo_catalog(str(catalog_path))
...
def sort_positions_lagrangian(
    ...
    return _ts.density.sort_by_lagrangian_id(positions, particle_ids, grid_size, id_offset)
...
def check_tessera_available() -> bool:
    ...
    _get_tessera()  # Trigger lazy import
    return HAS_TESSERA
